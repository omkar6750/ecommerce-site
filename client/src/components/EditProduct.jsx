...
const EditProduct = () => {
	...
	const handleNewVariant = () => {
		if (SKU.length > 0 && size && colour && inventory) {
			setVariant((prev) => [
				...prev,
				{
					sku: SKU,
		...
		} else toast.error("Add a new variant first");
	};
	...
	return (
		<div className="flex w-screen flex-col items-center justify-center p-2">
			<div className="m-5 border p-2">Search bar</div>
			<div className="flex h-[50vh] w-full flex-col overflow-y-auto border">
				{products.map((item) => {
					return <div key={item.product_id}>{item.name}</div>;
				})}
			</div>
